...
//	private Optional<Patient> optional;
	...
	public Optional<Patient> update(Patient patient) {
		Optional<Patient> optional = repository.findById(patient.getPatient_id());
	...
	public PatientBean getById(Long id) {
		...
		for (PatientRequest patientRequest : patientRequests) {
			PatientRequestBean patientRequestBean = PatientRequestBean.builder()
					.patient_request_id(patientRequest.getPatient_request_id())
					.patient_medical_condition(patientRequest.getPatient_medical_condition())
					.patient_request_on_date(patientRequest.getPatient_request_on_date())
					.patient_request_units(patientRequest.getPatient_request_units())
					.status(patientRequest.getStatus()).build();
			patientRequestBeans.add(patientRequestBean);
		}

		String Userurl = "http://localhost:8083/user/" + patient.getUser_id();
...
//		userBean.setBlood_group_id(userBean.getBlood_group_id());
		...
		PatientBean patientbean = PatientBean.builder().patient_id(patient.getPatient_id()).user_details(userBean)
