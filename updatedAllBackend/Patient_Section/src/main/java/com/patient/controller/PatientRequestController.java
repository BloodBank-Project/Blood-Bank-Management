...
public class PatientRequestController {
	...
	public ResponseEntity<List<PatientRequest>> getAllPatientRequest() {
		log.info("Going to Fetch Data ");
		try {
		List<PatientRequest> list = patientRequestService.getAllPatientRequest();
		ResponseEntity<List<PatientRequest>> responseEntity = new ResponseEntity(list, HttpStatus.OK);
		log.info("Fetched Successfully");
		return responseEntity;
		}
		catch (PatientRequestNotFoundException e) {
			log.error("Error Ocuured while Fetching");
			return new ResponseEntity<List<PatientRequest>>(HttpStatus.NOT_ACCEPTABLE);
		}
	}
	
	@GetMapping("/get/{id}")
	public ResponseEntity<PatientRequest> getPatientRequest(@PathVariable Long id){
		log.info("Going To Get Patient Details");
		try {
		PatientRequest patientRequest = patientRequestService.getPatientRequest(id);
		ResponseEntity<PatientRequest> patient=new ResponseEntity<>(patientRequest,HttpStatus.OK);
